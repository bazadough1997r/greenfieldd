...
con.connect(function (err) {
  ...
  var Cars =
    "CREATE TABLE IF NOT EXISTS cars (brand VARCHAR(20), year YEAR, price INT, colour VARCHAR(20), description VARCHAR(250), image TEXT, id INT PRIMARY KEY )";
  con.query(Cars, function (err, result) {
    if (err) throw err;
    console.log("Cars Table created!!!");
  });
  var Users =
    "CREATE TABLE IF NOT EXISTS users (id int NOT NULL AUTO_INCREMENT, firstName VARCHAR(20), lastName VARCHAR(20), username VARCHAR(20), email VARCHAR(30), password VARCHAR(255), PRIMARY KEY (id))";
